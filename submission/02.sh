#!/usr/bin/env bash
# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25
# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""

transaction=""

TXID=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.txid')

# Both outputs of the given tx are needed as inputs (neither alone covers 0.2 BTC)
# vout 0 first, then vout 1 — no explicit sequence, so Core's default (RBF, 0xfffffffd) applies
INPUTS="[{\"txid\":\"$TXID\",\"vout\":0},{\"txid\":\"$TXID\",\"vout\":1}]"

# Current block height is 25; 2 weeks = 2016 blocks (144 blocks/day * 14 days)
CURRENT_BLOCK=25
LOCKTIME=$((CURRENT_BLOCK + 2016))

RECIPIENT="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
AMOUNT="0.20000000"

RAW_TX=$(bitcoin-cli -regtest createrawtransaction \
  "$INPUTS" \
  "{\"$RECIPIENT\":$AMOUNT}" \
  $LOCKTIME)

echo "$RAW_TX"