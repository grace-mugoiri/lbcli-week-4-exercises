# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
transaction=""
DECODED_TRANSACTION=$(bitcoin-cli -regtest decoderawtransaction $transaction)
TXID=$(echo $DECODED_TRANSACTION | jq -r '.txid')
VOUT=$(echo $DECODED_TRANSACTION | jq -r '.vout[0].n')
CURRENT_BLOCK=25
LOCKTIME=$((CURRENT_BLOCK + 2016))
RECEIVER_ADDRESS="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
AAMOUNT="0.2000000"
RAW_TRANSACTION=$(bitcoin-cli -regtest createrawtransaction "[{\"txid\":\"$TXID\",\"vout\":$VOUT}]" "{\"$RECEIVER_ADDRESS\":$AAMOUNT}" $LOCKTIME)
echo "Raw transaction created: $RAW_TRANSACTION"
